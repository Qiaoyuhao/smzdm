...
public class NewsController {
    ...
    public HashMap addLike(String newsId, HttpSession session) {
        ...
        if (null == user) {
            HashMap map = ResultUtils.resultMsg(1, "请登录后操作");
            return map;
        } else {
            User userBySession = (User) user;
            int like = newsService.addLike(Integer.valueOf(newsId), userBySession.getId());
            HashMap map = ResultUtils.resultMsg(0, like);
            return map;
        }
    ...
    public HashMap addDislike(String newsId, HttpSession session) {
        ...
        if (null == user) {
            ...
            User userBySession = (User) user;
            int like = newsService.addDislike(Integer.valueOf(newsId), userBySession.getId());
            HashMap map = ResultUtils.resultMsg(0, like);
            return map;
        }
    }
